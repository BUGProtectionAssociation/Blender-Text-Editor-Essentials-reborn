...
class PMM_OT_PIPInstall(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            # TODO: Check if the operation is finished
            # If finished, call self.finish(context)
            return {'PASS_THROUGH'}
...
class PMM_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col1.label(text="PIP modules:")
        col1.label(text="Module Name(s):")
        col2.operator(PMM_OT_PIPList.bl_idname, text="List")
        col2.prop(bpy.context.scene, "pip_module_name" ,text='')
        row = layout.row(align=True)
